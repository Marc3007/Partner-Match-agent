...
export interface MatchResult {
  ...
  /**
   * One plain-language sentence explaining why THIS vendor could address
   * the stated problem -- distinct from `reason`, which explains ranking
   * (why it did/didn't make top-3), not capability fit. Only populated for
   * `top` results; null for alternatives/belowConfidence. Generated by
   * lib/semanticClassifier.ts's generateMatchRationales() strictly AFTER
   * scoring/ranking is final, so it is context only and never a scoring
   * input (see runMatch() in lib/matching.ts). Falls back to a
   * deterministic, tag-based sentence rather than staying null when the
   * generation call is unavailable or fails.
   */
